    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Role: {{ role.display_name || role.name }}</h5>
...
// Initialize permissions object from role data
const permissions = ref({});
props.modules.forEach(module => {
    permissions.value[module.id] = {
        read: props.role.read?.includes(module.id) || props.role.read?.includes('*') || false,
        create: props.role.create?.includes(module.id) || props.role.create?.includes('*') || false,
        update: props.role.update?.includes(module.id) || props.role.update?.includes('*') || false,
        delete: props.role.delete?.includes(module.id) || props.role.delete?.includes('*') || false
    };
});
...
const submit = () => {
    ...
    form.put(route('permission.roles.update', props.role.id), {
        onSuccess: () => redirect()
    });
};
</script>
